...
import { COURSE_LANGUAGES, COURSE_LEVELS, COURSE_TOOLS, courseCategoryFromValue } from '@/lib/courses/constants'
    ...
    <article className="group flex h-full flex-col rounded-[28px] border border-gray-200 bg-white p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)] transition-all hover:-translate-y-0.5 hover:border-gray-300 hover:shadow-[0_18px_36px_rgba(0,0,0,0.08)]">
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-gray-100 pt-4">
        <span className="flex items-center gap-2 text-sm">
          <span className={course.price === 'gratis' ? 'font-semibold text-workcofy-green' : 'font-semibold text-gray-800'}>
            {course.price === 'gratis' ? 'Gratis' : course.price_text ?? 'Pago'}
          </span>
          {course.has_certificate && (
            <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-semibold text-gray-600">Certificado</span>
          )}
        </span>
        <a
