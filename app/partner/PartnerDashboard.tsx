...
import { createPartnerPromotion, deletePartnerPromotion, updatePartnerPromotion, updatePartnerSocials, validateBenefitCode } from './actions'
...
function CodeValidator() {
  ...
  return (
    <section className="rounded-3xl border border-gray-200 p-5">
      ...
      <p className="mt-1 text-xs text-gray-500">El código de 6 dígitos que el miembro te muestra al hacer check-in. Es de un solo uso y vence en 10 minutos.</p>
...
export function PartnerDashboard({ space, promotions = [] }: { space: any; promotions?: any[] }) {
  ...
  async function remove(id: string) { if (!confirm('¿Eliminar esta promoción?')) return; try { await deletePartnerPromotion(space.id, id); location.reload() } catch (e) { setMessage(e instanceof Error ? e.message : 'No se pudo eliminar') } }
  return <div className="mt-8 space-y-5">
    ...
    <section className="rounded-3xl border border-gray-200 p-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-gray-400">Promociones</p><input value={title} onChange={e => setTitle(e.target.value)} placeholder="Ej. Americano + sandwich" className="mt-4 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Condiciones y precio especial" className="mt-3 min-h-24 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /><p className="mt-3 text-xs text-gray-500">Las fechas se pueden ajustar después de publicar.</p><button onClick={publish} className="mt-3 rounded-full bg-workcofy-yellow px-4 py-2 text-sm font-bold text-black">Publicar promoción</button>{promotions.length > 0 && <div className="mt-5 space-y-2 border-t border-gray-100 pt-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-gray-400">Gestionar promociones</p>{promotions.map((promotion) => <div key={promotion.id} className="flex items-start justify-between gap-3 rounded-xl bg-gray-50 p-3"><div><p className="text-sm font-bold">{promotion.title}</p><p className="text-xs text-gray-500">{promotion.description || 'Sin condiciones adicionales'}</p></div><button onClick={() => remove(promotion.id)} className="text-xs font-bold text-red-600">Eliminar</button></div>)}</div>}</section>
