...
export default async function CheckInPage({ params }: { params: { slug: string } }) {
  ...
  return <main className="mx-auto min-h-screen max-w-md bg-white px-5 pb-10 pt-7">
    ...
    <section className="mt-8 overflow-hidden rounded-[28px] bg-black p-6 text-white shadow-xl shadow-black/10">
      ...
      <CheckInButton spaceId={space.id} spaceName={space.name} benefitLabel={benefits[0]?.label} className="mt-6 flex w-full items-center justify-center rounded-full bg-workcofy-yellow px-5 py-3.5 text-sm font-bold text-black transition-transform active:scale-[0.98]" />
      <p className="mt-3 text-center text-[11px] text-white/40">Tu acceso es válido una vez al día en este local</p>
    </section>
    <section className="mt-9">
      <div className="flex items-end justify-between"><h2 className="text-xl font-extrabold tracking-tight">Beneficios disponibles</h2><span className="text-xl">🎁</span></div>
