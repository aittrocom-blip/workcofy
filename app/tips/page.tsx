import { listPublishedTips, tipsOfTheDay, TIP_CATEGORY_LABELS, type TipCategory } from '@/lib/data/tips'
...
export default async function TipsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-10 pt-6 md:px-8 md:pt-10">
      ...
      <p className="mt-2 max-w-md text-sm text-gray-600">Consejos cortos para tu día remoto. Tres nuevos cada día en Explorar.</p>

      {today.length > 0 && (
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {today.map((tip) => (
            <div key={tip.id} className="rounded-[24px] bg-black p-5 text-white">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-workcofy-yellow">Hoy · {TIP_CATEGORY_LABELS[tip.category]}</p>
              <h2 className="mt-2 text-base font-extrabold leading-snug tracking-tight">{tip.title}</h2>
              <p className="mt-2 text-xs leading-relaxed text-white/80">{tip.body}</p>
            </div>
          ))}
        </div>
      )}
